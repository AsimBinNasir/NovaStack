            ...
            <div className="container mx-auto px-6 flex justify-between items-center">
                ...
                <nav className="hidden md:flex items-center space-x-8">
                    {navItems.map((item) => {
                        ...
                        // Determine the width class based on active state or hover
                        const widthClass = isActive ? 'w-full' : 'w-0';
                ...
                <div className="md:hidden absolute top-full left-0 w-full bg-nova-dark/95 backdrop-blur-xl border-b border-white/10 p-6 flex flex-col space-y-4">
                    {navItems.map((item) => (
                        <a
                            key={item.name}
                            onClick={(e) => handleNavigation(e, item.path)}
                            // Example of active styling for mobile menu
                            className={location.pathname === item.path ? 'text-nova-peach font-bold' : 'text-white/80'}
                        >
                            {item.name}
                        </a>
                    ))}
